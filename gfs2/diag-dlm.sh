...
systemctl status dlm-controld --no-pager -n0 >> "$LOG_DIR/01_services_status.txt" 2>&1
systemctl status lvmlockd   --no-pager -n0 >> "$LOG_DIR/01_services_status.txt" 2>&1
echo

# 2) Logs do journalctl (últimos 100 registros) para DLM e lvmlockd
echo "==> 2) Logs do journalctl (dlm-controld)" | tee "$LOG_DIR/02_journal_dlm.txt"
journalctl -u dlm-controld -n 100 --no-pager >> "$LOG_DIR/02_journal_dlm.txt" 2>&1
echo

echo "==> Logs do journalctl (lvmlockd)" | tee "$LOG_DIR/03_journal_lvmlockd.txt"
...
if command -v dlm_tool &>/dev/null; then
  sudo dlm_tool dump >> "$LOG_DIR/04_dlm_tool_dump.txt" 2>&1 || true
else
...
pcs status --no-quorum --full >> "$LOG_DIR/09_pcs_status.txt" 2>&1 || true
echo

# 8) Versões dos pacotes relevantes
echo "==> 8) Versões dos pacotes" | tee "$LOG_DIR/10_versions.txt"
dpkg -l | grep -E 'dlm-controld|lvm2|corosync|pacemaker' >> "$LOG_DIR/10_versions.txt" 2>&1 || true
echo
