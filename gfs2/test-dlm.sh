...
# 2) Listar lockspaces existentes
print_header "2) Listando lockspaces (dlm_tool)"
if command -v dlm_tool &>/dev/null; then
  sudo dlm_tool dump && print_success "dlm_tool executado com sucesso" || print_error "Erro no dlm_tool dump"
else
  print_error "dlm_tool não encontrado; instale o pacote dlm-utils"
fi
...
sudo pcs resource delete test-dlm && print_success "Recurso 'test-dlm' removido" || print_error "Falha ao remover recurso 'test-dlm'"
